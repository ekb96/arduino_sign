// при подаче питания: ардуино ожидает инициализации датчика наклона
...
// при входящем звонке с нужного номера запоминает начальный угол наклона
// отслеживает изменение угла от начального на определенный угол до момента повторного входящего с того же номера
// производит исходящий на заданный номер в случае отклонения от начального угла



// нужно реализовать программный ресет GSM и ARDUINO в случае тайм-аута,
...
// и выход из него по событию из модуля GSM (ЗАПУСК СИГНАЛИЗАЦИИ)
// и уход в спящий режим по событию из модуля GSM (ОТКЛЮЧЕНИЕ СИГНАЛИЗАЦИИ)
...
int index = 5;                                  // Индекс для цикла мигания свеодиодом
...
long lat = 0, lon;                              // Переменные для широты и долготы
bool newdata = false;
...
void setup()                                    // Начальные установки
{
  ...
  while(1)
  {
    Two_Serial.println("AT+COPS?");
    if(Two_Serial.find("+COPS: 0"))
    {
...
void loop()                                                 // Основной цикл
{
  while(play == 0)                                          // Если неактивна, то ждем входящего вызова
  {
    ...
    if(Two_Serial.find("RING"))                             // Если входящий вызов
    {
      ...
      if (innerPhone.length() >= 7 && List_Phones.indexOf(innerPhone) >= 0)
      {
        ...
        indicator();                                                    // Цикл мигания светодиодом
        ...
        if( ay >= 0)
        {
          angle_ax0 = 90 - TO_DEG*acos(ay);
        }
        else
        {
          angle_ax0 = TO_DEG*acos(-ay) - 90;
        }
  ...
  while(play == 1)                                                   // Если активна, то ждем входящего вызова
  {
    ...
    if( t_next < t )
    {
      ...
      Serial.print(" Angle: "); Serial.println(angle_ax);           // вывод в порт угла поворота вокруг оси X
